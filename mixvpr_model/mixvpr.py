...
class MatchingPipeline:
    ...
    def run(self,query_dataset:SceneDataset,db_dataset:SceneDataset,top_k:int=10)->Dict[str,List[str]]:
        ...
        database_pipeline = InferencePipeline(model=self.model, dataset=db_dataset, feature_dim=4096, device=self.device)
        query_pipeline = InferencePipeline(model=self.model, dataset=query_dataset, feature_dim=4096, device=self.device)
...
class InferencePipeline:
    ...
    def __init__(
        self, model:VPRModel, 
        dataset:SceneDataset, 
        feature_dim:int, batch_size:int=4, num_workers:int=4, device:str='cuda'
    ):
        self.model = model
