...
class GsvDatasetPartial(SceneDataset):
    ...
    def __init__(
        ...
        # Setup
        self.mode = mode
        # /content/drive/MyDrive/Dataset/gsv-cities/Images/Bangkok
    ...
    def __getitem__(self, name_idx: Union[str, int]) -> Tuple[Scene, int]:
        ...
        intrinsics_matrix, width, height = self.read_intrinsics(
            str(self.data_path / name), self.resize
        )
        ...
        return (
            Scene.create_dict(
                name, image, depth, intrinsics_matrix, q, t, width, height
            ),
            index,
        )
